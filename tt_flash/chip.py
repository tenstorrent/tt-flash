...
class TTChip:
    ...
    def board_type(self):
        if self.use_luwen:
            ...
            # UMD's TTDevice has no pci_board_type(); the UPI lives in bits 36-55
            # of the board id, which is what get_board_type(from_type=True) expects.
            return (self.chip.get_board_id() >> 36) & 0xFFFFF

    def board_id(self) -> int:
        if self.use_luwen:
            return PciChip(self.interface_id).board_id()
        else:
            return self.chip.get_board_id()
